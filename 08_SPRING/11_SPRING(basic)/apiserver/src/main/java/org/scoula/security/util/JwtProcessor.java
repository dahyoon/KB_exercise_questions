...
public class JwtProcessor {
    static private final long TOKEN_VALID_MILLISECOND = 1000 * 60 * 2L; // 2분

    private String secretKey = "충분히 긴 임의의(랜덤) 비밀키 문자열 배정";
    private Key key = Keys.hmacShaKeyFor(secretKey.getBytes(StandardCharsets.UTF_8));
    // private Key key = Keys.secretKeyFor(SignatureAlgorithm.HS256); -- 운영시 사용

    // JWT 생성
    public String generateToken(String subject) { // subject 는 username
        return Jwts.builder()
                .setSubject(subject)
